...
В любой процедуре Доп обработчика можно переопределить параметры чека, вызвав их через ```ОсновнаяОбработка```:
...
- ```ДанныеДляПечатиЧекаДокумента``` - функция, возвращает структуру следующего вида: 
    ...
    ```
  Ее можно использовать как образец для передачи поле, которые необходимо переопределить при использовании функции [```ОбработатьУниверсальныйДок```](#обработатьуниверсальныйдок).
