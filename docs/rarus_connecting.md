...
Инструкция описывает подключение [ККТ-ОНЛАЙН 54ФЗ](https://yadi.sk/d/jcdpGNZ43WQtro) к типовому механизму оборудования Рарус.
...
> Команды печати запускаются из привычного интерфейса Рарус, но чек заполняет обработка. Она получает данные из фронт-кассира или документа основания. Перед первой продажей проверьте товары, оплаты и маркировку в предварительном чеке.
...
- скорректируем справочник **Оборудование**, чтобы можно было добавлять внешние обработки для работы с ТО:
  ...
  - в форме элемента необходимо заменить переменную ```глРарус_Компонента``` на нашу ```глРарус_КомпонентаТО```, она встречается только одни раз в процедуре ```ОсновныеДействияФормыОборудование``` и будет выглядеть так
    ...
    - и в самом конце модуля формы элемента, до области инициализации модуля, добавить следующий код:
        ...
        Процедура СохранениеОбработкиВКаталогНажатие(Элемент)
	          ККТ_РаботаСТорговымОборудованием.СохранитьОбработкуВКаталогКнопка(ЭтотОбъект);	
        КонецПроцедуры
...
- доработаем модуль объекта обработки "ФронтКассира":
  ...
  - Для Альфа-авто 5 современных версий, нужно также закомментировать механизм проверки маркировки - так как он выполняется в самой обработке непосредственно.
    Нужно в процедуре  ```ДействиеПробить``` закомментировать вызов запуска проверки маркировки:
...
## Проверка после подключения Рарус

Сначала проверьте получение состояния ККТ. Затем откройте предварительный чек из используемого документа и из фронт-кассира, если он применяется. Сверьте источник товаров и оплат: параметр «Игнорировать данные Фронт-кассира» меняет источник товарных строк, но оплаты продолжают поступать из фронт-кассира.

Отдельно проверьте маркировку, терминал и завершение смены. Если после обновления конфигурации изменились формы или процедуры, перечисленные в инструкции, доработку должен проверить программист. Не заменяйте отсутствующие участки кода по сходству названий.
