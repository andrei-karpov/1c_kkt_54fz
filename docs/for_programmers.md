...
Раздел описывает передачу документа в обработку и расширение её функций через дополнительный обработчик.


## Доработка конфигурации ##

При подключении как «ККТ с передачей данных» документ передаётся предусмотренным конфигурацией механизмом. В других случаях обработка может пытаться определить его по журналу регистрации, что занимает время и не всегда даёт нужный результат. Для явной передачи документа используйте [«ВПФ_Чек»](connecting.md#структура-архива-с-обработкой), если конфигурация поддерживает внешние печатные формы, либо выполните описанную ниже доработку.

Пример доработки показан в видео: [Доработка конфигурации](https://www.youtube.com/watch?v=Ehp2DU-YnqM&index=1&list=PLv043XNq9i-6_DdLAy1kTuExX2E-ikn65).
...
Если шаблону чека или собственному коду нужна ссылка на объект базы, задайте её в дополнительных параметрах обработки. Получить значение в коде можно так:

```bsl
ЗначениеПараметра = ОсновнаяОбработка.ККТ_ЗначениеДополнительногоПараметра("ИмяПараметра");
```

Откройте форму «Дополнительные параметры» в настройках обработки.

![Вызов дополнительные параметры](media/вызов_доп_параметры.png)

Укажите имя параметра и его значение.

![Дополнительные параметры](media/f59e12e9afcf868c7f4bb320fc094d87.png)


## Изменение конфигурации для поддержки НДС 5% и НДС 7% ##

В конфигурациях Рарус, где ставки НДС хранятся в справочнике, новую ставку добавляют как элемент справочника. Для конфигураций с перечислением ставок может потребоваться доработка. Состав изменений зависит от конкретного релиза; проверьте все места расчёта, передачи и отображения НДС, включая расчётные ставки.

[Доработка 1С для использования ставок НДС 5% и 7% УСН (не для программистов)](https://dzen.ru/video/watch/676845df343a2f3f490e13d5)

## Изменение функционала "под себя" ##

Для изменения поведения печати используйте дополнительный обработчик из комплекта поставки: [KKT_DEVELOPE82](connecting.md#структура-архива-с-обработкой) для 1С 8.2/8.3 или [KKT_DEVELOPE81](connecting.md#структура-архива-с-обработкой) для 1С 8.1. Укажите путь к нему в параметрах основной обработки. Собственный код размещайте в предусмотренных экспортных процедурах и функциях.

Пример таких изменений описан в [видео](https://youtu.be/5t4pye9cRd8) (_устарело_)
...
- Через формы-модули можно вызывать экспортные методы. Например, стандартные параметры чека возвращает вызов ```ОсновнаяОбработка.мМодульККТОбщий.ПараметрыОперацииФискализацииЧека()```. Чтобы посмотреть код модуля, откройте основную обработку в конфигураторе и выберите соответствующую форму:
...
## События дополнительного обработчика

Ниже перечислены дополнительные точки расширения шаблона версии 4.16. Все процедуры должны быть экспортными. Параметры передаются по ссылке; изменяйте входные данные и предусмотренные флаги отказа с учётом этапа обработки.

| Событие | Момент вызова и назначение |
| --- | --- |
| `ДоОперацииПоЭквайрингу` | До диалога и подключения терминала. Позволяет изменить входные параметры или отказаться от штатной операции. Введённые позднее значения диалога могут заменить сумму и вид оплаты |
| `ПослеОперацииПоЭквайрингу` | После успешной операции и предусмотренной печати слипа. При ошибке этих этапов вызов не выполняется |
| `ДоФискализацииЧека` | После формирования XML, перед вызовом драйвера. Позволяет изменить XML или пропустить штатную печать |
| `ПослеФискализацииЧека` | После ветки фискализации. При отрицательном ответе драйвера не вызывается, но может вызываться при отказе от штатной печати в предыдущем событии |
| `ДоОткрытияСмены`, `ДоЗакрытияСмены` | Перед штатной операцией со сменой; доступны изменение параметров и отказ |
| `ПослеОткрытияСмены`, `ПослеЗакрытияСмены` | После обращения к драйверу, до проверки его булева результата. Переданное состояние является XML-строкой ответа драйвера |
| `ПередОтчетомБезГашения` | Перед X-отчётом; доступен отказ от штатного выполнения |
| `ПередЧекомКоррекции` | После подготовки XML коррекции; доступны изменение данных и отказ |
| `ПослеЧекаКоррекции` | После обращения к драйверу, до проверки результата операции |
| `ПередОтправкойЧека` | Отдельно перед EMAIL и SMS средствами обработки. Отказ относится к конкретной отправке, а не к рассылке ОФД |
| `ПередОбработкойШтрихкода` | Перед штатным разбором введённого кода при наличии текущей строки. Позволяет изменить данные или отказаться от штатной обработки |
| `ПередЗаполнениемТаблицыОплат` | Перед стандартным заполнением оплат. Можно заполнить таблицу самостоятельно и отключить стандартный алгоритм |

Событие с названием «После…» не является универсальным подтверждением успешной фискальной или банковской операции. Используйте результат основного вызова и возвращённые данные. Исключения собственного обработчика перехватываются обёрткой и записываются в лог; исключение не заменяет предусмотренный флаг отказа.

Точные сигнатуры дополнительных процедур:
...
Процедура ДоФискализацииЧека(пПараметры, пОбщиеПараметры, пПараметрыПодключения,
    пДанныеЧека, ОтказатьсяОтПечати, НомерЧека, НомерСмены, ФискальныйПризнак) Экспорт
КонецПроцедуры

Процедура ПослеФискализацииЧека(пПараметры, пОбщиеПараметры,
    пТекстЭлектронногоЧека, НомерЧека, НомерСмены, ФискальныйПризнак) Экспорт
КонецПроцедуры

Процедура ДоОткрытияСмены(пВходныеПараметры, пПараметры, пПараметрыПодключения,
    ОтказатьсяОтПечати, НомерСмены) Экспорт
КонецПроцедуры

Процедура ПослеОткрытияСмены(пВходныеПараметры, пПараметры, пПараметрыПодключения,
    пПараметрыСостояния, НомерСмены, НомерЧека) Экспорт
КонецПроцедуры

Процедура ДоЗакрытияСмены(пВходныеПараметры, пПараметры, пПараметрыПодключения,
    ОтказатьсяОтПечати, НомерСмены, НомерЧека) Экспорт
КонецПроцедуры

Процедура ПослеЗакрытияСмены(пВходныеПараметры, пПараметры, пПараметрыПодключения,
    пПараметрыСостояния, НомерСмены, НомерЧека) Экспорт
КонецПроцедуры

Процедура ПередОтчетомБезГашения(пВходныеПараметры, пПараметры,
    пПараметрыПодключения, ОтказатьсяОтПечати) Экспорт
КонецПроцедуры

Процедура ПередЧекомКоррекции(пВходныеПараметры, пПараметры, пПараметрыПодключения,
    пДанныеЧека, ОтказатьсяОтПечати, НомерЧека, НомерСмены, ФискальныйПризнак) Экспорт
КонецПроцедуры

Процедура ПослеЧекаКоррекции(пВходныеПараметры, пПараметры, пПараметрыПодключения,
    НомерЧека, НомерСмены, ФискальныйПризнак) Экспорт
КонецПроцедуры

Процедура ПередОтправкойЧека(СсылкаНаДокумент, ДанныеОтправкиЧека,
    ОтказатьОтОтправки = Ложь) Экспорт
КонецПроцедуры

Процедура ПередОбработкойШтрихкода(ФормаОбработки, СтрокаСМаркировкой,
    ВведенныйШтрихкод, ОтказатьсяОтОбработки = Ложь) Экспорт
КонецПроцедуры

Процедура ПередЗаполнениемТаблицыОплат(СсылкаНаДокумент, ТаблицаОплаты,
    СоответствиеОплат, ОтказатьсяОтЗаполнения = Ложь) Экспорт
КонецПроцедуры
```

В поставляемом шаблоне для 8.1 отсутствует `ПередЗаполнениемТаблицыОплат`, хотя основной модуль уже поддерживает его вызов. При необходимости добавьте экспортную процедуру с указанной сигнатурой. Остальные перечисленные сигнатуры совпадают в проверенных шаблонах 8.1 и 8.2.

Проверяйте доработку на копии базы и эмуляторе, затем на используемой компоненте. Особенно проверяйте отказ от штатной операции, ошибки драйвера и повторный запуск после частично выполненной операции.
