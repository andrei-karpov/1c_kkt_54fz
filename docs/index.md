...
> Если установленная обработка не подключается к серверу лицензирования, скачайте доступную вашей лицензии сборку через [форму лицензирования](licensing.md#форма-лицензирования). Для замены адреса сервера также опубликовано [обновление](https://disk.yandex.ru/d/VoqPUzNsOmZJaw). Перед заменой файла выполните [подготовку к обновлению](connecting.md#подготовка-к-обновлению-и-переносу).
...
- [Подключить обработку и проверить первый чек](connecting.md#проверка-первого-подключения).
- [Оформить продажу или возврат](form_check_and_check_correction.md#продажа-и-проверка-результата).
- [Настроить маркировку](marking.md#порядок-проверки-маркированного-товара).
- [Подключить ТС ПИоТ: требования, порты и настройка](ts_piot.md).
- [Разобраться с ошибкой после печати](errors.md#чек-напечатался-но-в-1с-возникла-ошибка).
- [Обновить обработку](connecting.md#как-обновить-ранее-приобретенную-программу) или [перенести настройки](parameters_description.md#что-сохраняется-в-файл-настроек).

Обработка для 1С 8.2 подходит и для 1С 8.3 на обычных формах. Для 1С 8.1 используется отдельный файл. Перед установкой проверьте [состав поставки](connecting.md#структура-архива-с-обработкой) и способ подключения в вашей конфигурации.
...
- Поддерживает разрешительный режим продажи маркированного товара ([Настройка разрешительного режима продажи](marking.md#разрешительный-режим-продажи-маркировки))
- Поддерживает локальный модуль офлайн проверки маркировки в рамках разрешительного режима ([Настройка локального модуля (офлайн проверка)](marking.md#настройка-локального-модуля-для-офлайн-проверки))
...
- поддерживает **[ставку НДС 22%](instruction.md#поддержка-ндс-22)**
- поддерживает новый механизм работы [проверки маркировки **ТС ПИоТ**](ts_piot.md)
...
- [**Маркировка. Продажа лекарств, шин, обуви и т.д.**](marking.md)
    ...
    - [**Настройка ТС ПИоТ**](ts_piot.md)
        - [Требования к обработке, драйверам и прошивкам](ts_piot.md#requirements)
        - [Порты и схема обмена](ts_piot.md#ports)
        - [Настройка подключения в обработке](ts_piot.md#setup)
        - [Проверка работы и диагностика](ts_piot.md#verification)
