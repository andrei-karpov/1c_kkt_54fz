...
- **«Сформировать файл технической поддержки»** создаёт файл, по которому обработка проверяет доступность версии при отсутствии или нестабильности интернета. Сформируйте его на компьютере с интернетом, перенесите на рабочее место и укажите в поле [«Путь к файлу технической поддержки»](parameters_description.md#основные-параметры). Файл общий для лицензий одной учётной записи.
...
Откройте «Ручное управление» → «Написать разработчику». Укажите email, пароль учётной записи и текст обращения, затем нажмите «Отправить». Ответ придёт на указанную почту.
...
## Проверка лицензии после активации и переноса

Лицензия на рабочее место бессрочная, а доступ к новым версиям зависит от срока технической поддержки. Установка версии, не входящей в этот срок, может переключить обработку в деморежим. Доступную версию и сведения о лицензиях проверьте в форме лицензирования.

После активации убедитесь, что выбран режим «Лицензия», путь к ключу заполнен и файл доступен пользователю, под которым работает 1С. На рабочем месте без интернета также нужен файл технической поддержки. Файл активации, ключ лицензии и файл технической поддержки имеют разное назначение и не заменяют друг друга.

После переноса на другой компьютер проверьте активацию отдельно. Файл `.s1c` не переносит лицензию или путь к ней. Для фермы RDP-серверов сохраняется требование доступа к серверу лицензирования через интернет.

## Что приложить к обращению в поддержку

Укажите версию обработки, платформы и конфигурации 1С; модель ККТ, версию компоненты, драйвера и ФФД; способ подключения и использование RDP. Опишите действие перед ошибкой, точное время и текст сообщения.

Сообщите, был ли сформирован чек на ККТ, появились ли его реквизиты в 1С и прошла ли банковская операция. Приложите относящийся к операции лог. Не включайте в текст обращения пароли и ключи доступа; при наличии таких данных в материалах согласуйте их передачу с поддержкой.
