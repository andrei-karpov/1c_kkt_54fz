...
Таблицы ниже помогают распознать файлы компонентов, но не заменяют актуальный дайджест производителя. Отсутствие компоненты в таблице не означает отсутствия поддержки оборудования. Перед установкой сверяйте версию, разрядность клиента 1С, операционную систему и требуемую ревизию.

Для НДС 22% обработке нужна ревизия 4007 или выше; компоненты с меньшей ревизией в таблице не подходят для этой функции. Для маркировки и ТС ПИоТ отдельно проверьте прошивку, ФФД и совместимость сервиса. Выбор компоненты описан в [мастере подключения](connecting.md#мастер-подключения-компонент-начиная-с-версии-410).
