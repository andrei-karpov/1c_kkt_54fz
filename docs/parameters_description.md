...
**Ревизия** определяет команды и реквизиты, которые поддерживает компонента оборудования. Это не версия драйвера или прошивки. Требуемая ревизия зависит от функции: для НДС 22% нужна 4007 или выше. Описание интерфейсов опубликовано на [ИТС](https://its.1c.ru/db/metod8dev/content/4829/hdoc).
...
**Описание** - более подробная информация о компоненте оборудования. Например, для оборудования ВикиПринт в этом поле отображается история изменений компоненты оборудования.
...
**«Система налогообложения»** определяет СНО чека. Поле обязательно для заполнения и задаёт значение по умолчанию для [распределения по СНО](mechanism_distribution.md#распределение-по-системам-налогообложения). Проверьте, что выбранная СНО зарегистрирована на ККТ.
...
- Указать вручную – данные о кассире будут взяты из параметров обработки:
  ...
  - **Не передавать ИНН Кассира** - отключает передачу ИНН совсем, требуется в случае, если ИНН только с нулями не проходит формато логический контроль в ОФД.
...
**«Касса не поддерживает НДС 22% (ждем прошивку)»** - если для кассы еще не вышли нужные драйвера для поддержки ставки НДС 22%, то включите данный флаг. При таком параметре обработка вместо ставки 22% на кассу будет передавать ее старое значение - 20%

**«Подменять ставку НДС 20 на 22 с начала года»** - Параметр предназначен для автоматического перехода на новую ставку. Если в документе или номенклатуре указана ставка **НДС 20%** или **18%**, то при формировании чека она **автоматически заменяется на НДС 22%**.
...
**Сворачивать одинаковые строки при печати (с ревизии 4001)** передаёт кассе команду объединять одинаковые строки на бумажном чеке. На электронный чек настройка не влияет. Применимость к маркированным позициям зависит от компоненты и ККТ; проверьте результат на используемом оборудовании.
...
**Часовая зона (с ревизии 4006)** - применяется с ревизии драйверов 4006, на кассу передается тег 1011, отвечающие за заполнение часовой зоны (Timezone). Содержит смещение часового пояса от UTC. Если не задан, часовую зону должен передавать драйвер.
...
**«Испольузется оплата через СБП (если поддерживается)» -** при включении данного параметра появляется новый вид оплаты "СБП" в форме проверки печати, в основном он не отличается от безналичной оплаты, однако дополнительно в компоненту эквайрингового терминала передается, что это именно оплата через СБП.
...
- «ЗАМЕНИТЬ НА АВАНС» - заменяет все фискальные строки на текст «Оплата по документу основание» и признак способа расчета «Аванс» и признак предмета расчета «Платеж», если используется НДС, то ставка ндс будет заменена на ее эквивалент для авансовых платежей например 22/122, 5/105 или 7/107

- «ЗАМЕНИТЬ НА ЧАСТИЧНУЮ ОПЛАТУ» - в каждой фискальной строке количество становится равно 1, цена становится равна сумме фактической оплаты в поле Признак предмета расчета устанавливается "Платеж", в поле "Признак способа расчета" - "Предоплата частичная", если используется НДС, то будет заменен на его эквивалент для авансовых платежей, например НДС 22/122, 5/105 или 7/107

- «ЗАМЕНИТЬ НА "НОМЕНКЛАТУРА ПО УМОЛЧАНИЮ"» - заменяет все фискальные строки на текст из реквизита «Номенклатура по умолчанию», Признак способа расчета «Передача с полной оплатой» и признак предмета расчета «Товар»
...
**«Разрешительный режим»** открывает настройки проверки маркированного товара. Для работы через ЕСМ перейдите к [настройке ТС ПИоТ](ts_piot.md#setup): там описаны адрес подключения, дата перехода, сведения о кассе и настройки по организациям. Прямой обмен с Честным знаком описан в разделе [«Разрешительный режим продажи маркировки»](marking.md#разрешительный-режим-продажи-маркировки).
...
**«Ждать ответа ОИСМ при проверке маркировки»** при включении явно запрашивает ожидание ответа через ККТ для ФФД 1.2. Снятый флаг сам по себе не отключает ожидание: обработка обычно не передаёт этот признак, и поведение зависит от компоненты. При успешной проверке разрешительного режима и включённом параметре «Не выполнять проверку маркировки средствами ККТ» обработка явно передаёт запрос без ожидания. Отсутствие ожидания не подтверждает успешную онлайн-проверку; см. [значение результатов проверки](marking.md#порядок-проверки-маркированного-товара).
...
**«Сохранить настройки в файл»** сохраняет переносимую часть параметров обработки в `.s1c`. Файл не содержит полную настройку рабочего места. Перед переносом проверьте [состав сохраняемых данных](#что-сохраняется-в-файл-настроек).

**«Восстановить настройки из файла»** загружает переносимые параметры из `.s1c`. После загрузки проверьте поля, сохраните настройки и перезапустите 1С. Параметры драйвера и исключённые из экспорта настройки потребуется проверить отдельно.
...
## Что сохраняется в файл настроек

Файл `.s1c` переносит настройки обработки, доступные для экспорта. Он не является резервной копией базы 1С, обработки, драйверов или лицензии.

| Данные | Поведение при экспорте и восстановлении |
| --- | --- |
| Параметры обработки, не входящие в исключения | Сохраняются в файл и применяются при восстановлении |
| Путь к ключу и тип лицензирования | Не переносятся |
| Версия ФФД и дополнительное оборудование | Не переносятся |
| Тип оборудования, путь к компонентам, использование COM-обёртки | Не переносятся |
| Путь к собственному обработчику | Не переносится; сам файл доработки также нужно сохранить отдельно |
| Включение логирования и путь к логу | Не переносятся |
| Флаги «Аппарат без фискального накопителя» и открытия денежного ящика из 1С | Не переносятся |
| Параметры драйвера, включая физическое подключение оборудования | Не восстанавливаются из файла |

При восстановлении обработка также приводит загруженные параметры к формату текущей версии. Сообщение «Настройки восстановлены» подтверждает загрузку файла, но не проверку связи с ККТ.

После переноса проверьте лицензию, выбранную компоненту, связь с основной и дополнительными кассами, терминалы, СНО и НДС. Для маркировки проверьте организацию, ККТ, ФН и адрес сервиса. Порядок подготовки описан в разделе [обновления и переноса](connecting.md#подготовка-к-обновлению-и-переносу).
