...
Что такое ТС ПИоТ - это отдельное программное обеспечение, которое работает с кассой напрямую. Предполагается, что он заменит прямые обращения к серверу Честного знака, для проверки продажи маркированного товара. Дополнительно он требует отдельной лицензии на его использование. 
Ближайшая аналогия, что из себя представляет ТС ПИоТ это прокаченный локальный модуль, но с возможностью онлайн работы. Как его использовать, обычно он устанавливается как отдельное программное обеспечение (хотя ходят слухи, что возможно его вариант реализую сразу в 1С), подключается к кассе, для этого на кассе должны быть установлены прошивка и драйвера совместимые с ТС ПИоТ, при этом могут быть казусы, например драйвера АТОЛ 10.10.7.21 содержат специальное поле "порт gRPC", которое позволяет работать с сервисом, но эти драйвера не поддерживают НДС 22%, а драйвера 10.10.8.2, уже не поддерживают, зато работают с НДС, в общем пока это все очень сырое.

### Требования для использования ТС ПИоТ

Для использования разрешительного режима (ТС ПИоТ) ваша касса должна использовать: 

- формат фискальных данных **ФФД 1.2**.
- Иметь актуальную версию драйвера и прошивки вашего фискального регистратора.
- Версия данной программы не ниже **4.08**
- установлен и проверен модуль ЕСМ (ТС ПИоТ)
- настроена связь фискального регистратор и ТС ПИоТ

### Как обработка работает с ТС ПИоТ (ЕСМ).

Для использования ТС Пиот в меню разрешительного режима появилась возможность указать настройки ТС ПИот, а также, дату его применения, по умолчанию она устанавливается на 28 декабря 2025, однако ее можно сдвинуть для проверки работы заранее, либо до конца марта 2026, так как на текущий сохраняется техническая возможность использовать предыдущий разрешительный режим, а функционал ТС Пиот еще довольно сырой. 

![Настройка разрешительного режима](media/edit_tspiot.png)

В остальном для пользователя функционал работает аналогично предыдущему варианту. То есть, при продаже маркированного товара, теперь марка не уходит напрямую в Честный знак, а передается на ТС ПИоТ, и из него уже возвращаются результаты проверки. Далее информация о проверке уходит на кассу.

> **Обратите внимание**, что если вы перейдете на использование только ТС ПИоТ, то настраиваеть локальный модуль внутри обработки - не нужно, так как функционал 
работы в офлайн режиме включен в модуль ТС ПИоТ, и он интегрируется с локальным модулем напрямую, и сам должен контролировать связь и хранить офлайн данные маркировки.

### Проверка работы через демо-сервер

Есть внешний сервис эмулирующий работу сервиса ТС ПИоТ его можно использовать для проверки работы разрешительного режима. 
> Официальный адрес: https://esm-emu.ao-esp.ru

его нужно будет указать в настройках ТС ПИоТ. В обработке его можно использовать вместе с эмулятором фискального регистратора.
