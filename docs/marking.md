...
- Проверить не заблокирован ли доступ к серверам Честного знака на вашем сервере, такэе сам Честный знак может блокировать доступ, если клиентское подключение выполняется через VPN
