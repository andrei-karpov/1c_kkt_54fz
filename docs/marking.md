...
**ТС ПИоТ (ЕСМ)** — это отдельное программное обеспечение, через которое выполняется проверка разрешения на продажу маркированного товара. Оно взаимодействует с фискальным регистратором и берет на себя обмен данными с системой **«Честный знак»**.

Иными словами, касса или учетная система больше не обращается к «Честному знаку» напрямую. Код маркировки передается в ТС ПИоТ, сервис выполняет проверку — онлайн или офлайн — и возвращает результат. После этого касса продолжает формирование чека, а результат проверки фиксируется в фискальных данных.

Таким образом, вся логика разрешительного режима сосредоточена в ТС ПИоТ:

- проверка кодов маркировки;
- взаимодействие с локальным модулем;
- хранение данных для офлайн-проверок;
- возврат результата в кассу.

> **Обратите внимание:** для использования ТС ПИоТ потребуется отдельная лицензия. Сервис устанавливается как самостоятельное программное обеспечение. Возможность его встраивания непосредственно в 1С обсуждается, но на данный момент официально не подтверждена.

По своему назначению ТС ПИоТ можно сравнить с расширенным локальным модулем. Главное отличие — поддержка централизованной онлайн-проверки и собственная логика работы с офлайн-данными.

---

### Что важно учесть перед переходом

Инфраструктура ТС ПИоТ пока находится в стадии развития, поэтому совместимость компонентов необходимо проверять особенно внимательно. Основная сложность сейчас связана с версиями драйверов ККТ.

Например, в драйвере **АТОЛ 10.10.7.21** присутствует поле для указания порта gRPC, необходимое для взаимодействия с ТС ПИоТ, но нет поддержки ставки НДС 22%. В версии **10.10.8.2**, наоборот, поддержка НДС 22% уже есть, однако интеграция с ТС ПИоТ может работать некорректно из-за отсутствия необходимых параметров.

> **Важно:** при выборе драйвера недостаточно проверить только поддержку ТС ПИоТ или только актуальную ставку НДС. Версия должна одновременно соответствовать обоим требованиям.

Перед переходом рекомендуется отдельно проверить связку из конкретной модели фискального регистратора, его прошивки, версии драйвера и установленного модуля ТС ПИоТ.
...
В настройках разрешительного режима предусмотрены два параметра: **адрес сервиса ТС ПИоТ** и **дата начала его использования**.

По умолчанию установлена дата **28 декабря 2025 года**. При необходимости ее можно перенести на более ранний срок, например для предварительного тестирования. Старый механизм разрешительного режима при этом можно использовать до октября 2026 года.

Переходный период предусмотрен потому, что прежний механизм пока остается технически доступным, а функциональность ТС ПИоТ продолжает дорабатываться.

![Настройка разрешительного режима ТС ПИоТ](media/edit_tspiot.png)

Также добавлена возможность при ошибке проверки ТС Пиот, запускать проверку средствами разрешительного режима, если он настроен и работает. За это отвечает параметр "**При ошибке ТС Пиот проверять через разрешительный режим**"


Для заполнения настроек достаточно указать адрес ТС ПИоТ и нажать кнопку **«Заполнить настройки»**.

Если оборудование уже зарегистрировано в самом модуле ТС ПИоТ, обработка автоматически получит номер фискального накопителя и ИНН организации. Вводить эти сведения вручную не потребуется.

---

### Что изменится для пользователя

Для кассира привычный сценарий продажи не меняется. Работа с маркированным товаром и интерфейс кассы остаются прежними.

Изменяется только технический маршрут проверки: код маркировки передается не напрямую в систему «Честный знак», а в ТС ПИоТ. После проверки сервис возвращает результат, и продажа продолжается в обычном порядке.

---

### Офлайн-режим

> **Важно:** при использовании ТС ПИоТ не нужно отдельно настраивать локальный модуль маркировки внутри обработки.

Офлайн-режим уже реализован в самом ТС ПИоТ. Сервис самостоятельно управляет подключением, хранит необходимые офлайн-данные и напрямую взаимодействует с локальным модулем маркировки.

Повторная настройка локального модуля в обработке не требуется и может привести к конфликтам.

### Возможные ошибки при проверке марки через ТС ПИоТ

Ошибки `X-Fn-Sid`, `403 Forbidden` и `found:false` могут быть связаны с временной недоступностью CDN или восстановлением контролируемого канала. Сами по себе они не всегда означают, что марка отсутствует в системе маркировки.

| Ошибка | Возможная причина | Действия кассового ПО |
|--------|-------------------|-----------------------|
| `X-Fn-Sid` | ТС ПИоТ не смог получить или использовать идентификатор защищенной сессии: канал еще не установлен, SID пустой или стал недействительным, отсутствует связь с ККТ, ФН либо CDN | Повторить проверку через ТС ПИоТ через 2 секунды. При повторной ошибке выполнить еще одну попытку через 5 секунд |
| `403 Forbidden` | CDN получил запрос, но отклонил текущий `X-Fn-Sid`. Обычно это означает рассинхронизацию или потерю серверной сессии контролируемого канала | Дать ТС ПИоТ восстановить контролируемый канал и повторить исходную проверку|
| `Код маркировки не найден` | Из-за таймаута или проблемы связи ТС ПИоТ не получил полноценный онлайн-ответ и вернул результат локальной офлайн-проверки. В таком ответе наличие марки в ГИС МТ не подтверждается, поэтому `found` может принять значение `false` | Считать наличие марки неопределенным и повторить проверку. Если связь не восстановилась, обработать результат по штатному сценарию офлайн-проверки |

#### Ошибка `X-Fn-Sid`

`X-Fn-Sid` — идентификатор сессии контролируемого канала между ТС ПИоТ и CDN. Кассовое ПО этим идентификатором не управляет.

Ошибка может возникнуть, если:

* контролируемый канал еще не установлен;
* SID не получен из фискального накопителя;
* SID стал недействительным на стороне CDN;
* отсутствует связь ТС ПИоТ с ККТ или фискальным накопителем;
* недоступен API авторизации CDN;
* произошла ошибка шифрования или расшифрования данных в ФН.

Рекомендуемое сообщение пользователю: **«Не удалось установить защищенное соединение с системой маркировки. Повторите проверку»**.

#### Ошибка `403 Forbidden`

`403 Forbidden` возвращает непосредственно CDN. Запрос дошел до сервера, но авторизация по переданному `X-Fn-Sid` была отклонена. Чаще всего это означает, что серверная сессия была инвалидирована либо контролируемый канал рассинхронизировался.

ТС ПИоТ должен самостоятельно переустановить канал и получить новый SID. Кассовому ПО следует повторить проверку после небольшой паузы. Ошибка `403 Forbidden` не является результатом проверки самой марки.

Рекомендуемое сообщение пользователю: **«Система маркировки отклонила авторизацию. Выполняется переподключение»**.

#### `Код маркировки не найден` при проблеме связи

Если ТС ПИоТ не получил полноценный ответ от CDN из-за таймаута, недоступности сети или восстановления контролируемого канала, он может вернуть результат локальной офлайн-проверки. Локальный модуль не подтверждает наличие марки в ГИС МТ, поэтому поле `found` может принять значение `false`.
В этом случае `found:false` означает, что онлайн-статус марки не определен, а не то, что марка действительно отсутствует.

Кассовому ПО следует:
1. Считать результат неопределенным, а не отрицательным.
2. Повторить проверку через ТС ПИоТ через 2 секунды.
3. При повторной проблеме выполнить еще одну попытку через 5 секунд.
4. Не заменять `found:false` на `true` самостоятельно.
5. Если связь не восстановилась, использовать штатный сценарий офлайн-проверки.
