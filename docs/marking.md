# Общие сведения о маркировке {#маркировка}
...
Настройка ЕСМ и обработки описана в отдельной главе основной инструкции: [«Настройка ТС ПИоТ»](ts_piot.md). Для этой инструкции требуется **ККТ-ОНЛАЙН 4.17 или выше**.

<a id="что-такое-тс-пиот-и-как-он-работает"></a>
<a id="что-изменится-для-пользователя"></a>

Обработка передаёт код маркировки в ТС ПИоТ, получает результат проверки и использует его при формировании чека. Печать выполняется через компоненту и драйвер ККТ. Связи между обработкой, ЕСМ, кассой и системой маркировки показаны на [схеме обмена](ts_piot.md#flow).

<a id="что-важно-учесть-перед-переходом"></a>
<a id="что-потребуется-для-работы"></a>

Перед установкой сверьте [модель кассы, прошивку, драйвер и версии компонентов](ts_piot.md#requirements). Для АТОЛ, POScenter, ШТРИХ/ТБМ и Дримкас приведены отдельные требования и шаги. Назначение портов и проверка конфликтов описаны в [таблице портов](ts_piot.md#ports).

<a id="настройка-тс-пиот-в-обработке"></a>

После установки и регистрации ЕСМ выполните [настройку подключения в обработке](ts_piot.md#setup), затем [проверьте сведения о кассе, код маркировки и печать](ts_piot.md#verification). Одной успешной проверки подключения недостаточно.

<a id="офлайн-режим"></a>

Офлайн-проверкой через ТС ПИоТ управляет ЕСМ. Порядок настройки и отличие от локального модуля обычного разрешительного режима описаны в разделе [«Офлайн-проверка»](ts_piot.md#offline).

<a id="возможные-ошибки-при-проверке-марки-через-тс-пиот"></a>
<a id="ошибка-x-fn-sid"></a>
<a id="ошибка-403-forbidden"></a>
<a id="код-маркировки-не-найден-при-проблеме-связи"></a>

Ошибки `X-Fn-Sid`, `403 Forbidden`, `found:false`, недоступность ЕСМ и занятые порты разобраны в [диагностике ТС ПИоТ](ts_piot.md#diagnostics). Ошибка обмена не является разрешением на продажу.

<a id="как-проверить-работу-без-реального-оборудования"></a>

Для предварительной проверки логики используйте [тестовый сервис и эмулятор ККТ](ts_piot.md#emulator). Проверку реального оборудования проводят отдельно.
...
Эти настройки относятся к прямому обмену обработки с локальным модулем Честного знака в обычном разрешительном режиме. Модуль используется, когда онлайн-проверка недоступна. При подключении через ТС ПИоТ настройте [офлайн-режим самого сервиса](ts_piot.md#offline).
...
При работе через ТС ПИоТ эти параметры не заменяют настройку офлайн-режима самого сервиса. Различия маршрутов описаны в [разделе ТС ПИоТ](ts_piot.md#offline).
