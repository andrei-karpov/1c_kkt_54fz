...
1. формат фискальных данных **ФФД 1.2**.

2. Иметь актуальную версию драйвера и прошивки вашего фискального регистратора.

3. Версия данной программы не ниже **3.79**

4. Должен быть интернет на рабочем месте кассира.
...
- **Разрешить продажу при ошибке в разрешительном режиме** - позволяет пропустить проверку в разрешительном режиме в случае ошибки, при этом программа вначале попытается проверить через Честный Знак, а затем предложит пропустить проверку. Данная настройка не отключает механизм проверки через ККТ. 
   > **Обратите внимание**, что пропускать проверку товара в разрешительном режиме можно только при недоступности сервера Честного знака.
