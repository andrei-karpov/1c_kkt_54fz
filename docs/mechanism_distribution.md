...
> Для распределения обработка должна получить документ, по которому формируется чек. Если документ не определён, программа предложит выбрать его вручную. Способы передачи документа описаны в разделе [доработки конфигурации](for_programmers.md#доработка-конфигурации).
...
Приоритет зависит от того, применяется правило к документу целиком или к отдельной позиции. Порядок разделов ниже не определяет приоритет. Под «параметром» понимается настраиваемое значение: СНО, ставка НДС, признак предмета или способа расчёта и другие реквизиты.

## Приоритет правил для документа и позиции

Для документа условия проверяются в порядке: «Все документы» (`ALL`), тип документа, организация, подразделение, контрагент, произвольный код документа. Каждое найденное значение заменяет предыдущее. Поэтому правило контрагента имеет преимущество перед правилом организации, а результат произвольного кода документа применяется последним, если он определён.

Для позиции используется первое найденное значение:

1. Характеристика.
2. Номенклатура, включая подходящую группу.
3. Вид маркировки.
4. Вид номенклатуры.
5. Номенклатурная группа.
6. Тип справочника.
7. Склад.
8. Произвольный код позиции.
9. Значение по умолчанию, если ни одно условие не вернуло результат.

Например, для номенклатуры задана ставка 22%, а для её характеристики 10%. У позиции с этой характеристикой будет выбрано 10%. Произвольный код позиции не переопределит результат уже совпавшего условия.

В таблицах отбора номенклатуры и контрагентов используется первая подходящая строка, в том числе при совпадении по группе. Если одно правило задано для папки, а другое для элемента этой папки, проверьте порядок строк.

Если применяется неожиданное значение, проверьте документ основания, характеристику и номенклатуру позиции, затем остальные условия по указанному порядку. После изменения настроек перезапустите 1С и проверьте предварительный чек.
...
Укажите тип документа, для которого действует правило. Например, можно задать ставку НДС для документа «Корректировка долга». Правило применяется до формирования таблицы чека; настройки организации, подразделения, контрагента и произвольный код документа могут его переопределить.
...
Чтобы выбрать допустимое значение `ПризнакРаспределения`, откройте контекстное меню редактора: «Вставить» → «Возможный вариант распределения».
...
Обработка определяет номер таможенной декларации и код страны происхождения по номенклатуре и передаёт их в ОФД. Чтобы исключить эти реквизиты для выбранных условий, снимите флажок «Печатать». Условия задаются через [Механизм распределения](#механизм-распределения).

## Настройка печати получателя и его ИНН ##

![Настройка печати получателя и его ИНН](media/a1713c9def569c4b41f6ab8ff7d9af5d.png)

Обработка определяет данные контрагента и его ИНН и передаёт их в ОФД. Чтобы исключить сведения о покупателе для выбранных условий, снимите флажок «Печатать». Условия задаются через [Механизм распределения](#механизм-распределения).

## Настройка ограничения времени продажи ##

![Настройка ограничения времени продаж](media/77da70edc4b67687b401032a88194ac1.png)

Задайте время, в которое разрешена продажа товара, например алкоголя. Условия настраиваются через [Механизм распределения](#механизм-распределения).
...
Можно заменить названия выбранных позиций номенклатуры одним наименованием. Условия задаются через [«Механизм распределения»](#механизм-распределения).
