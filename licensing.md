...
Одним из магазинов, где представлена программа является ООО "**Инфостарт**". С этим ресурсом есть ряд сложностей по активации лицензии.
После покупки разработчику не предоставляется данные покупателя, поэтому автоматическое прикрепление заказов не работает, из-за того, что нет информаци об email
Клиенту необходимо самостоятельно прикрепить заказ с сайта к своему email.

Для этого в [форме первоначальной настройки](connecting.md#Подключение):
- нажмите **Получить ключ** - откроется форма лицензирования
- укажите адрес электронной почты ![Прикрепление заказа Инфостарт](media/add_order_infostart.png)
